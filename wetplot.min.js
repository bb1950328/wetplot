...
d)*d;g<e+this._config.time_length;g+=d){var h=new Date(1E3*g),l=Math.round(this._seconds_to_x_coords(g));h=h.toLocaleString();h=h.substring(0,h.lastIndexOf(":"));var k=createSvgElement("text");k.innerHTML=h;k.setAttribute("x",0);k.setAttribute("y",f/4);k.setAttribute("transform","rotate(90,0,0) translate(10 "+-l+")");k.style.fontSize=f+"px";k.classList.add("xAxisText");a.appendChild(k);c+="M"+l+" 0 V "+this._config.height}b.setAttribute("d",c)};
...
Wetplot.prototype._get_min_max_for_unit=function(a){var b=this,c=$jscomp.makeIterator([void 0,void 0]),d=c.next().value,e=c.next().value;this.getAllLineCodes().forEach(function(f){if(b._line_config[f].unit===a){var g=$jscomp.makeIterator([b._line_config[f].min,b._line_config[f].max]);f=g.next().value;g=g.next().value;void 0===d?(d=f,e=g):(d=Math.min(f,d),e=Math.max(g,e))}});return[d,e]};
Wetplot.prototype._count_units=function(){var a=this,b=[];this.getAllLineCodes().forEach(function(c){c=a._line_config[c].unit;-1===b.indexOf(c)&&b.push(c)});return b.length};
Wetplot.prototype._add_y_axis_for_line_if_needed=function(a){var b=this._line_config[a].unit,c="group"+this._unit_to_id(b);if(-1===this._created_y_axes.indexOf(c)&&this._yAxisElement){this._created_y_axes.push(c);var d=this._config.font_size_px;this.getAllLineCodes();this._yAxisElement.setAttribute("width",4*this._count_units()*d);var e=this._created_y_axes.length-1;console.log("creating y axis for unit "+b+";"+e);var f=this._yAxisElement.getElementById(c);f&&(this._yAxisElement.removeChild(f),console.debug("removed old y axis with id "+
c));f=createSvgElement("g");f.setAttribute("id",c);f.setAttribute("fill",this._line_config[a].color);this._yAxisElement.appendChild(f);c=createSvgElement("text");f.appendChild(c);c.innerHTML=b;c.style.fontSize=d+"px";c.setAttribute("y",d);c.setAttribute("x",(4*e+.5)*d);b=$jscomp.makeIterator(this._get_min_max_for_unit(b));var g=b.next().value;b=b.next().value;c=1.2*d/(this._config.height/(b-g));b=0;if(.5>c){for(b=1;.05>c;)b++,c*=10;c=(.1>c?.1:.2>c?.2:.5)/Math.pow(10,b-1)}else c=Math.ceil(c);if(0===
...
e}};Wetplot.prototype._seconds_to_x_coords=function(a){return Math.round((a-this._config.time_offset)/this._config.seconds_per_pixel)};Wetplot.prototype._x_coords_to_seconds=function(a){return a*this._config.seconds_per_pixel+this._config.time_offset};Wetplot.prototype._value_to_y_coord=function(a,b){var c=$jscomp.makeIterator(this._get_min_max_for_unit(this._line_config[a].unit));a=c.next().value;c=c.next().value;return this._config.height-(b-a)/(c-a)*this._config.height};
Wetplot.prototype._y_coord_to_value=function(a,b){var c=$jscomp.makeIterator(this._get_min_max_for_unit(this._line_config[a].unit));a=c.next().value;c=c.next().value;return(this._config.height-b)/this._config.height*(c-a)+a};
