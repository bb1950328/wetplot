...
this._line_config={"###default###":{type:"line",color:"#000000",line_width:2,name:"?",unit:"1",auto_min_max:!1,min:-1,max:25,visible:!0}};this._data=null;this._x_offset=0;this._created_y_axes=[];this._data_callback=null};
...
d)*d;g<e+this._config.time_length;g+=d){var h=new Date(1E3*g),k=Math.round(this._seconds_to_x_coords(g));h=h.toLocaleString();var l=h.lastIndexOf(":");h=h.substring(0,l)+h.substring(l+3,h.length);l=createSvgElement("text");l.innerHTML=h;l.setAttribute("x",0);l.setAttribute("y",f/4);l.setAttribute("transform","rotate(90,0,0) translate(10 "+-k+")");l.style.fontSize=f+"px";l.classList.add("xAxisText");a.appendChild(l);c+="M"+k+" 0 V "+this._config.height}b.setAttribute("d",c)};
...
Wetplot.prototype._rebuildLine=function(a){var b=this._data.heads.indexOf(a),c=this._data.heads.indexOf("Time"),d=this._line_config[a],e="",f=!1;if(d.visible){if(d.auto_min_max){var g=$jscomp.makeIterator(this._data.getMinMaxForColumn(b)),h=g.next().value;g=g.next().value;var k=(g-h)/100;d.min=h-k;d.max=g+k}if("line"===d.type)for(e="M",h=!0,g=0;g<this._data.values.length;g++){k=this._seconds_to_x_coords(this._data.values[g][c]);var l=this._value_to_y_coord(a,this._data.values[g][b]);h?h=!1:e+=" L";
e+=" "+Math.round(k)+" "+Math.round(l)}else if("ybar"===d.type){f=!0;e=void 0;l=g=this._config.time_offset;var m=k=0;h=[];for(var p=0;p<this._data.values.length;p++){var q=this._data.values[p][c],n=Math.floor(q/this._config.seconds_per_grid_line);void 0!==e&&e!==n&&(h.push([g,l,k]),g=q,m=Math.max(k,m),k=0);k+=this._data.values[p][b];e=n;l=q}d.auto_min_max&&(d.min=0,d.max=1.01*m);b=this._value_to_y_coord(a,0);e="M 0 "+b;for(c=0;c<h.length;c++)g=h[c][0],l=h[c][1],k=h[c][2],k=this._value_to_y_coord(a,
k),g=this._seconds_to_x_coords(g)+1,l=this._seconds_to_x_coords(l)-1,e+=" H "+g,e+=" V "+k,e+=" H "+l,e+=" V "+b;e+=" Z"}else console.error("Unknown type for line "+a+': "'+d.type+'"')}console.debug(e);b=document.getElementById("path"+a);null===b&&(b=createSvgElement("path"),this._svgElement.appendChild(b),b.classList.add("linePath"),b.setAttribute("id","path"+a));b.style.stroke=d.color;b.style.strokeWidth=d.line_width;f&&(b.style.fill=d.color,b.style.fillOpacity=.4);b.setAttribute("d",e)};
...
"valuesPopupBg"),f=createSvgElement("text"),f.setAttribute("id","valuesPopupTime"),f.style.fontSize=this._config.font_size_px+"px",e.appendChild(f)):e.style.display="inline";this.getVisibleLineCodes().forEach(function(t){t="valuesPopup"+t;var u=document.getElementById(t);null==u&&(u=createSvgElement("text"),u.style.fontSize=c._config.font_size_px+"px",u.classList.add("valueText"),u.setAttribute("id",t),e.appendChild(u))});var k=h(.25),l=h(1),m=(new Date(1E3*d)).toLocaleString();m=m.substring(0,m.lastIndexOf(":"));
f.innerHTML=getText("TIME")+": "+m;f.setAttribute("y",l);m=f.innerHTML.length;var p=this._seconds_to_values(d,!0);d=e.getElementsByClassName("valueText");for(var q=0;q<d.length;q++){var n=d.item(q),r=n.id.substring(11),v=this._line_config[r].name+": "+Math.round(100*p[r])/100+" "+this._line_config[r].unit;n.innerHTML=v;m=Math.max(m,v.length);n.style.fill=this._line_config[r].color;l+=h(1);n.setAttribute("y",l)}m=2*Math.ceil(m/4.5);l+=h(.25);p=h(this.getVisibleLineCodes().length+1.25);q=(this._config.height+
p)/2;.5<b/this._config.width?(e.setAttribute("transform","translate("+(a-h(m+1))+", "+q+")"),g.setAttribute("transform","")):(e.setAttribute("transform","translate("+a+", "+q+")"),g.setAttribute("transform","rotate(180) translate("+h(-(m+1))+" "+-1*p+")"),k+=h(1));f.setAttribute("x",k);for(a=0;a<d.length;a++)d.item(a).setAttribute("x",k);g.setAttribute("d","M 0 0 H "+h(m)+" V "+.4*l+" L "+h(m+1)+" "+.5*l+" L "+h(m)+" "+.6*l+" V "+l+" H 0 V 0")};
...
Wetplot.prototype._count_visible_units=function(){var a=this,b=[];this.getVisibleLineCodes().forEach(function(c){c=a._line_config[c].unit;-1===b.indexOf(c)&&b.push(c)});return b.length};
Wetplot.prototype._add_y_axis_for_line_if_needed=function(a){var b=this,c=this._line_config[a].unit,d="group"+this._unit_to_id(c);if(this._yAxisElement){var e=this._yAxisElement.getElementById(d);e&&(this._yAxisElement.removeChild(e),this._created_y_axes=this._created_y_axes.filter(function(v){return v!==d}),console.log("removed old y axis with id "+d))}e=0<this.getVisibleLineCodes().filter(function(v){return b._line_config[v].unit===c}).length;console.log("any_visible_with_same_unit="+e+"    unit=",
c);if(e&&-1===this._created_y_axes.indexOf(d)&&this._yAxisElement){this._created_y_axes.push(d);var f=this._config.font_size_px;this._yAxisElement.setAttribute("width",4*this._count_visible_units()*f+"px");var g=this._created_y_axes.length-1;console.log("creating y axis for unit "+c+";"+g);e=createSvgElement("g");e.setAttribute("id",d);e.setAttribute("fill",this._line_config[a].color);this._yAxisElement.appendChild(e);var h=createSvgElement("text");e.appendChild(h);h.innerHTML=c;h.style.fontSize=
f+"px";h.setAttribute("y",f);h.setAttribute("x",(4*g+.5)*f+"px");h=$jscomp.makeIterator(this._get_visible_min_max_for_unit(c));var k=h.next().value;h=h.next().value;var l=1.2*f/(this._config.height/(h-k));h=0;if(.5>l){for(h=1;.05>l;)h++,l*=10;l=(.1>l?.1:.2>l?.2:.5)/Math.pow(10,h-1)}else l=Math.ceil(l);if(0===l)console.warn("something is wrong, i can feel it");else{var m=this._config.height-f;for(k=Math.floor(k);this._value_to_y_coord(a,k)>m;)k+=l;var p=this._value_to_y_coord(a,k);m="";var q=p;for(g=
4*(g+.15)*f;p>2*f;){var n=p;var r=createSvgElement("text");r.innerHTML=this.numberToDisplayText(k,h);r.setAttribute("y",p+.4*f);r.setAttribute("x",g+f/3);r.style.fontSize=f+"px";e.appendChild(r);m+="M "+g+" "+p+" h "+(f/-3).toFixed(2)+" ";k+=l;p=this._value_to_y_coord(a,k)}m+="M "+g+" "+n+" V "+q;n=createSvgElement("path");n.setAttribute("d",m);n.setAttribute("id","yLadder"+a);n.setAttribute("stroke",this._line_config[a].color);e.appendChild(n)}}};
Wetplot.prototype._unit_to_id=function(a){for(a=encodeURI(a.replace("\u00b0","deg").replace("%","percent"));-1!==a.indexOf("%");)a=a.replace("%","");return a};Wetplot.prototype.config=function(a,b){void 0!==b&&(this._config[a]=b);return this._config[a]};Wetplot.prototype.addLine=function(a){if(a===DEFAULT_LINE_CODE)throw Error("invalid lineId");this._line_config[a]=Object.assign({},this._line_config[DEFAULT_LINE_CODE]);this._line_config[a].name=a;this._add_y_axis_for_line_if_needed(a)};
Wetplot.prototype.lineConfig=function(a,b,c){void 0!==c&&(this._line_config[a][b]=c);return this._line_config[a][b]};Wetplot.prototype.addWetplotData=function(a){this._addDataToDb("10min",a);this._data=null===this._data?a:WetplotData.concatenate(this._data,a)};Wetplot.prototype.usePalette=function(a){a=COLOR_PALETTES[a];var b=this.getAllLineCodes();if(a)for(var c=0;c<b.length;c++)this.lineConfig(b[c],"color",a[c%a.length])};
...
Wetplot.prototype._seconds_to_values=function(a,b){var c=this;b=void 0===b?!1:b;var d=$jscomp.makeIterator(this._nearestTwoDataRows(a)),e=d.next().value,f=d.next().value,g={};if(null!==e||null!==f){null===e?e=f:null===f&&(f=e);d=this._data.heads.indexOf("Time");var h=e[d],k=(a-h)/(f[d]-h);(b?this.getVisibleLineCodes():this.getAllLineCodes()).forEach(function(l){var m=c._data.heads.indexOf(l),p=e[m];g[l]=p+(f[m]-p)*k})}return g};Wetplot.prototype.scrollTo=function(a){this._moveViewBoxPixels(a,!0)};
...
Wetplot.prototype._show_buttons=function(){var a=this,b=[];if(this._config.show_line_visibility_button){var c=document.createElement("div");c.setAttribute("id","wetplot-line-toggle-container");c.style.display="none";c.style.position="absolute";c.style.backgroundColor="#cccccccc";this.getAllLineCodes().forEach(function(h){var k=document.createElement("div");k.style.color=a._line_config[h].color;k.setAttribute("id","toggle_"+h);k.innerText="\u2611"+a._line_config[h].name;k.addEventListener("click",
function(l){a.toggleLineVisibility(h);a._line_config[h].visible?(k.style.color=a._line_config[h].color,k.innerText="\u2611"+k.innerText.substring(1)):(k.style.color="#444444",k.innerText="\u25fb"+k.innerText.substring(1));a._redraw_all()});c.appendChild(k)});var d=document.createElement("div");d.innerText="\ud83d\udc41";d.alt=getText("VISIBLE_LINES");d.addEventListener("click",function(h){"none"===c.style.display?(c.style.display="block",h=d.getBoundingClientRect(),c.style.top=h.bottom+h.top+"px",
...
Wetplot.prototype._redraw_all=function(){var a=performance.now(),b=Math.floor(this._config.width/2),c=this._x_coords_to_seconds(this._x_offset+b);this._redraw_y_axis();this._redraw_x_axis();this._redraw_horizontal_grid();this._rebuildAllLines();this._moveViewBoxPixels(this._seconds_to_x_coords(c)-b,!0);b=performance.now();console.info("_redraw_all() took "+(b-a)/1E3+"s")};
...
WetplotData.concatenate=function(a,b){var c=new WetplotData;a.heads.forEach(c.heads.push);b.heads.forEach(function(n){-1===c.heads.indexOf(n)&&c.push(n)});var d=a.getValue(0,"Time"),e=b.getValue(0,"Time"),f=Math.min(d,e);e=d=0;var g={},h={};for(c.heads.forEach(function(n){g[n]=a.getColumnIndex(n);h[n]=b.getColumnIndex(n)});d<a.values.length||e<b.values.length;){var k=a.getValue(d,"Time")-f,l=b.getValue(e,"Time")-f;if(isNaN(l)||k<=l){var m=[],p=a.values[d],q=$jscomp.makeIterator(c.heads);for(f=q.next();!f.done;f=
q.next())m.push(p[g[f.value]]);c.values.push(m);f=a.getValue(d,"Time");d++}if(isNaN(k)||l<=k){k=[];l=b.values[e];m=$jscomp.makeIterator(c.heads);for(f=m.next();!f.done;f=m.next())k.push(l[h[f.value]]);c.values.push(k);f=b.getValue(e,"Time");e++}}return c};WetplotData._add_columns=function(a,b,c){c=void 0===c?null:c;var d=b.length;if(!(0>=d)){for(var e=a.values.length,f=0;f<e;f++)for(var g=0;g<d;g++)a.values[f].push(c);b.forEach(function(h){return a.heads.push(h)})}};
